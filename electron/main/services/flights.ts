...
async function resolvePlace(query: string, key: string, host: string): Promise<{ skyId?: string; entityId: string }> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 404) {
      throw new Error(
        `auto-complete 404 em ${host}. O host/endpoint não bate com a API Skyscanner assinada. ` +
          `URL: ${url}. Ajuste o host em Configurações (ex.: sky-scanner3.p.rapidapi.com) ou me diga qual API do RapidAPI você assinou.`
      )
    }
